...
export async function configureReasoningMode(): Promise<void> {
	...
	await Promise.all([
		...
		config.update(REASONING_SHOW_SUMMARY_KEY, summarySelection.value, vscode.ConfigurationTarget.Global),
		config.update(REASONING_PRESERVE_HISTORY_KEY, preserveSelection.value, vscode.ConfigurationTarget.Global),
	]);
